...
import type { ResumeSection, CustomEntry } from '@/types/resume';
...
function EntryCardContent({
  ...
  const entryLabel = entry.title || entry.subtitle || 'this entry';
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <TiptapEditor
              ...
              placeholder="Describe your project, certification, achievement..."
            />
          </div>
        </div>
      </div>
...
export function CustomSectionEditor({ section, onFieldFocus }: CustomSectionEditorProps) {
  const { dispatch } = useResumeData();
  ...
  const entries = section.entries.filter(
    (e): e is CustomEntry => e.type === 'custom'
  );
    ...
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-surface-100">
          <LayoutList className="h-5 w-5 text-surface-400" />
        </div>
        <p className="mb-1 text-sm font-medium text-surface-600">
          No entries yet
        </p>
        <p className="mb-4 text-xs text-surface-400">
          Add entries for projects, certifications, volunteer work, or anything else.
        </p>
        <button
          type="button"
          onClick={addEntry}
